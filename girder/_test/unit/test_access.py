...
def testGetFullAccessList(db, model, admin):
    acl = model('user').getFullAccessList(admin)
    assert len(acl['users']) == 1


def testAdminTokenScopes(server, model, admin, user):
    adminSettingToken = model('token').createToken(
        user=admin, scope=TokenScope.SETTINGS_READ)
    adminEmailToken = model('token').createToken(
        user=admin, scope=TokenScope.DATA_READ)
    nonadminToken = model('token').createToken(
...
def testArtificialScopedAccess(server, model, admin, user):
    ...
    # Test public access
    authToken = model('token').createToken(user=user)
...
def testRequiredScopeExists(server, model, user):
    token = model('token').createToken(scope=CUSTOM_SCOPE)
